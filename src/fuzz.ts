...
export type ReactComponentRenderStrategy<Props> =
  | ((props: Props) => unknown | Promise<unknown>)
  | {
...
const loadCorpus = <Props>(corpusPath: string | undefined): Props[] => {
  ...
  return payload.map((entry) => deserializeValue(entry) as Props);
};

const saveCorpus = <Props>(corpusPath: string | undefined, corpus: Props[]) => {
  if (!corpusPath) {
    return;
  }
...
const defaultRender = async <Props>(
  component: React.ComponentType<Props>,
  props: Props,
): Promise<void> => {
  renderToStaticMarkup(createElement(component as React.ComponentType<any>, props as any));
};

const resolveRender = <Props>(
  component: React.ComponentType<Props>,
  render: ReactComponentRenderStrategy<Props> | undefined,
) => {
  if (!render) {
    return (props: unknown) => defaultRender(component, props as Props);
  }
  if (typeof render === "function") {
    return (props: unknown) => render(props as Props);
  }
  return (props: unknown) => render.render(component, props);
};

const resolveInputDescriptor = <Props>(
...
export const sampleProps = async (
  options: SourceOptions & { numRuns?: number; seed?: number },
...
export const sampleBoundaryProps = async <Props>(
  options: SourceOptions & { maxCases?: number; render?: ReactComponentRenderStrategy<Props> },
): Promise<Array<Record<string, any>>> => {
  const descriptor = analyzePropsDescriptor({
    exportName: options.exportName,
    propsTypeName: options.propsTypeName,
  ...
  return boundaryValuesFromDescriptor(inputDescriptor, options.maxCases ?? 64) as Array<Record<string, any>>;
};
...
const clampBounds = (constraints: FuzzConstraints | undefined, defaults: [number, number]) => {
...
const generateString = (constraints: FuzzConstraints | undefined, rng: DeterministicRng): string => {
...
const generateUnknown = (rng: DeterministicRng): unknown => {
...
const generateValue = (descriptor: TypeDescriptor, rng: DeterministicRng): unknown => {
...
const matchesDescriptor = (value: unknown, descriptor: TypeDescriptor): boolean => {
  switch (descriptor.kind) {
    case "unknown":
      return true;
    case "string":
...
const coverageKeysForTarget = (coverage: ScriptCoverage[], sourcePath: string) => {
...
export const fuzzReactComponent = async <Props>(
  ...
  const arbitrary = arbitraryFromDescriptor(inputDescriptor) as fc.Arbitrary<unknown>;
  let lastProps: unknown;

  try {
    await fc.assert(
      fc.asyncProperty(arbitrary, async (props) => {
        lastProps = props;
        await render(props);
      }),
      {
...
export const fuzzReactComponentGuided = async <Props>(
  ...
  try {
    for (let iteration = 0; iteration < maxIterations; iteration += 1) {
      const props =
        corpus.length === 0 || iteration < initialCorpusSize
